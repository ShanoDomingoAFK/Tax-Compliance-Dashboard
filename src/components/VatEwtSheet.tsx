...
export default function VatEwtSheet({
  type,
  visibleTransactions,
  visibleLedgerRows,
  ...
  const purchaseSum = useMemo(() => {
    return visibleTransactions.reduce((sum, t) => sum + (type === 'vat' ? t.vat : t.ewtAmount), 0);
  }, [visibleTransactions, type]);

  const ledgerSum = useMemo(() => {
    return visibleLedgerRows.reduce((sum, r) => sum + r.amount, 0);
  }, [visibleLedgerRows]);

  const totalDiff = purchaseSum - ledgerSum;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="text-xs text-slate-500 bg-slate-50 border border-slate-200 rounded-xl p-3 shadow-inner">
        <strong>{label} Reconciliation</strong> compares Check Vouchers against G/L bank statements. Clear discrepancies using adjustments or verifying missing invoice attachments.
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-center bg-white/70 border border-slate-200/80 rounded-xl p-3 shadow-sm">
        <div className="relative flex-1 min-width-[280px]">
          ...
          <input
            type="text"
            placeholder="Search CV, supplier, account or reference code..."
            value={searchQuery}
        ...
        >
          <option value="">All {title} CVs</option>
          <option value="balanced">Balanced</option>
          <option value="unbalanced">Not balanced</option>
        </select>
        <button
          onClick={onClearMonthLedger}
          className="btn btn-danger text-xs font-bold py-2 px-4 rounded-full hover:bg-red-50 border-red-200"
        >
          🗑️ Clear Uploaded Ledgers for {activeMonthLabel}
        </button>
      </div>
